...
export class WebJobDiscovery {
  private exa: InstanceType<typeof Exa>;
   ...
   */
  async searchJobsWithExa(config: JobHuntConfig): Promise<DiscoveredJob[]> {
    ...
    try {
      ...
      for (const result of results.results) {
        ...
        if (config.excludeCompanies?.some(c =>
  ...
  async discoverCompanies(
    ...
    try {
      ...
      for (const result of results.results) {
        ...
        const careersUrl = await this.analyzer.findCareersPage(
          companyName,
          company.website
        );
        if (careersUrl) {
   ...
   */
  async scrapeCompanyCareersPage(
    page: Page,
    company: Company,
    searchQuery?: string
    ...
    try {
      ...
      const details = await page.evaluate(() => {
        ...
        return {
          ...
          applyUrl: document.querySelector<HTMLAnchorElement>(
   ...
   */
  private looksLikeJobPage(url: string, title: string, text: string): boolean {
    const jobKeywords = [
   ...
   */
  private extractCompany(url: string, title: string): string {
    try {
   ...
   */
  private jobMatchesQuery(title: string, query: string): boolean {
