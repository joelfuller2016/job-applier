...
export class JobHunterOrchestrator {
     ...
     */
    async hunt(userProfile, config, callbacks = {}) {
        const result = {
            sessionId: uuid(),
     ...
     */
    async discoverJobs(config, callbacks) {
        const jobs = [];
        // Search with Exa
     ...
     */
    async matchJobs(jobs, userProfile, config, callbacks) {
     ...
     */
    async applyToJob(job, userProfile, callbacks) {
        const attempt = {
            jobId: job.id,
            companyName: job.company,
            jobTitle: job.title,
        ...
        try {
            ...
            const formResult = await this.navigator.navigateMultiPageForm(page, async (analysis) => {
                const fillResult = await this.formFiller.fillForm(page, userProfile, jobContext, analysis);
     ...
     */
    async saveScreenshot(page, jobId) {
     ...
     */
    async quickApply(company, jobTitle, userProfile, callbacks = {}) {
        await this.browserManager.launch();
        try {
            // Find careers page
...
//# sourceMappingURL=job-hunter-orchestrator.js.map