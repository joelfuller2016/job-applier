 ...
 * SECURITY: Prevents IDOR vulnerabilities by ensuring users can only modify their own profiles
 * @throws TRPCError with NOT_FOUND if profile doesn't exist
 * @throws TRPCError with FORBIDDEN if user doesn't own the profile
 */
...
export const profileRouter = router({
  ...
  updateContactInfo: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      verifyProfileOwnership(ctx.profileRepository, input.id, ctx.userId);
  ...
  addSkill: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      verifyProfileOwnership(ctx.profileRepository, input.profileId, ctx.userId);
